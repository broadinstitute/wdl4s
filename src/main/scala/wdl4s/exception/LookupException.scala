...
/**
  ...
  */
case class VariableNotFoundException(variable: String) extends Exception(s"Variable '$variable' not found") with LookupException
...
/**
  ...
  */
case class ScatterIndexNotFound(message: String) extends VariableLookupException(message)

/**
  * Raised when attempting to resolve an output variable but the output resolver failed to return a value.
  */
case class OutputVariableLookupException(node: GraphNode, index: Option[Int]) extends VariableLookupException(s"Could not find outputs for call ${node.fullyQualifiedName} at index $index")