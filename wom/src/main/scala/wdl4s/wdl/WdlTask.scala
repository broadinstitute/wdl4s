...
import wdl4s.wom.callable.Callable.{DeclaredInputDefinition, OptionalInputDefinition, RequiredInputDefinition}
...
case class WdlTask(name: String,
  ...
  private def buildWomInputs: List[Callable.InputDefinition] = declarations collect {
    ...
    case d if d.expression.nonEmpty =>
      DeclaredInputDefinition(d.unqualifiedName, d.wdlType, WdlWomExpression(d.expression.get, Option(this)))
  } toList
}
